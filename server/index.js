...
if (process.env.AIRTABLE_API_KEY && process.env.AIRTABLE_BASE_ID && process.env.AIRTABLE_TABLE_NAME) {
  ...
  airtableTable = airtableBase(process.env.AIRTABLE_TABLE_NAME);
  console.log('✅ Airtable initialized successfully');
} else {
  console.log('⚠️  Airtable not configured - skipping Airtable sync');
}

// Helper function to clean values for Airtable (removes quotes)
const cleanValueForAirtable = (value) => {
...
// Sync application data to Airtable
const syncToAirtable = async (applicationData) => {
  if (!airtableTable) {
  ...
  try {
    const record = await airtableTable.create({
      full_name: cleanValueForAirtable(applicationData.full_name),
      email: cleanValueForAirtable(applicationData.email),
      phone: cleanValueForAirtable(applicationData.phone),
      experience_level: cleanValueForAirtable(applicationData.experience_level),
      current_occupation: cleanValueForAirtable(applicationData.current_occupation),
      why_join: cleanValueForAirtable(applicationData.why_join),
      payment_plan: cleanValueForAirtable(applicationData.payment_plan),
      payment_status: cleanValueForAirtable(applicationData.payment_status || 'pending'),
      razorpay_order_id: cleanValueForAirtable(applicationData.razorpay_order_id || ''),
      razorpay_payment_id: cleanValueForAirtable(applicationData.razorpay_payment_id || '')
    });
    console.log('✅ Synced to Airtable:', record.getId());
  } catch (error) {
...
const updateAirtableRecord = async (email, updates) => {
  if (!airtableTable) {
    console.log('⚠️  Airtable not configured - skipping update');
    return;
  }
  ...
  try {
    ...
    if (records.length > 0) {
      ...
      for (const [key, value] of Object.entries(updates)) {
        cleanedUpdates[key] = cleanValueForAirtable(value);
      }
      
      await airtableTable.update(recordId, cleanedUpdates);
      console.log('✅ Updated Airtable record for:', email);
    } else {
      console.log('⚠️  No Airtable record found for:', email);
    }
  } catch (error) {
...
app.post('/api/order', async (req, res) => {
  try {
    ...
    syncToAirtable(applicationRecord).catch(err => 
    ...
    if (signatureValid) {
      ...
      if (applicationId) {
        ...
        if (updateError) {
          ...
          // Get the email for Airtable update
          ...
          if (appData) {
            // Update Airtable record (non-blocking)
            updateAirtableRecord(appData.email, {
              payment_status: DEMO_MODE ? 'demo_completed' : 'completed',
              razorpay_payment_id: razorpay_payment_id
            }).catch(err => 
...
// Migrate all applications to Airtable
app.post('/api/admin/migrate-to-airtable', async (req, res) => {
  try {
    if (!airtableTable) {
      return res.status(400).json({
        success: false,
    ...
    for (let i = 0; i < applications.length; i += batchSize) {
      ...
      await Promise.all(batch.map(async (app) => {
        try {
          await syncToAirtable(app);
          synced++;
        } catch (error) {
          console.error(`Failed to sync application ${app.id}:`, error.message);
          failed++;
        }
      }));
